...
class HOTA(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        res['best_matches'] = [([], [])] * len(data['gt_ids'])
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            if len(gt_ids_t) == 0:
                ...
                res['best_matches'][t] = ([], tracker_ids_t)
                continue
            if len(tracker_ids_t) == 0:
                for a, alpha in enumerate(self.array_labels):
                    res['HOTA_FN'][a] += len(gt_ids_t)
                res['best_matches'][t] = (gt_ids_t, [])
                continue
            ...
            res['best_matches'][t] = (list(match_rows) + [i for i in list(range(len(gt_ids_t))) if i not in match_rows], list(match_cols) + [i for i in list(range(len(tracker_ids_t))) if i not in match_cols])
